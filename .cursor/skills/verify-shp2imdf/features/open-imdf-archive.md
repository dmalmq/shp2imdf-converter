...
- `imdf-open-file` loads an archive and navigates to `/review`.
...
- **Land on review.** Wait for `/review`. A toast `IMDF archive opened` reports a feature count. `Export` becomes available after load.
- **Bad file.** Set `#imdf-file-input` to a non-IMDF zip. An error region appears; URL stays `/`.
- **Proof.** For the control-only path, screenshot Import showing `Open IMDF archive`. For a full open, screenshot `/review` with a non-zero feature list and write `artifacts/verify-shp2imdf/open-imdf-archive/`.
