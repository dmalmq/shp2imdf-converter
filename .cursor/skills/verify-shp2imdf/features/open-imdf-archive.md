...
- `imdf-open-control` shows `Reopen an IMDF archive` on the hub (`/`).
...
- On the hub (`/`), under `Start something new`, choose `Reopen an IMDF archive`.
- Or drop the archive on the hub's drop zone: `.imdf`, `*.imdf.zip`, or a zip whose file list holds `manifest.json` and no `.shp`.
...
- **Control.** On `/`, the button `Reopen an IMDF archive` is enabled. Run `getByRole('button', { name: /Reopen an IMDF archive/ })`. Its formats line reads `.imdf · .zip`.
...
- **Proof.** For the control-only path, screenshot the hub showing `Reopen an IMDF archive`. For a full open, screenshot `/p/<sessionId>/check` with a non-zero feature list and write `artifacts/verify-shp2imdf/open-imdf-archive/`.

## Gotchas

- This is a different input from the hub's drop zone (`hub-drop-input`) and from Bring in's shapefile dropzone. Mixing them looks like “import did nothing” or “not an archive”.
