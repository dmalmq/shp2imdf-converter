...
def visit_gap() -> timedelta:
    raw = os.getenv("HANDOVER_VISIT_GAP_MINUTES", "").strip()
    try:
        minutes = float(raw) if raw else DEFAULT_VISIT_GAP_MINUTES
    except ValueError:
        minutes = DEFAULT_VISIT_GAP_MINUTES
    return timedelta(minutes=minutes if minutes > 0 else DEFAULT_VISIT_GAP_MINUTES)


def begin_or_continue_visit(handover: Handover, previous: datetime, now: datetime) -> None:
