...
function contents(overrides: Partial<Record<ExportFormat, Partial<ExportContents>>> = {}): ExportContents[] {
  const base: Record<ExportFormat, ExportContents> = {
    imdf: { format: "imdf", filename: "Tokyo_Station.imdf", entries: ["manifest.json", "venue.geojson", "unit.geojson"], unavailable: null, rows_skipped: [] },
    imdf_zip: { format: "imdf_zip", filename: "Tokyo_Station.zip", entries: ["manifest.json", "venue.geojson", "unit.geojson"], unavailable: null, rows_skipped: [] },
    shapefiles: { format: "shapefiles", filename: "Tokyo_Station_shapefiles.zip", entries: ["JRTokyoSta_B1_unit.shp", "JRTokyoSta_B1_unit.dbf", "export_report.json"], unavailable: null, rows_skipped: [] },
    odc2026_shapefiles: { format: "odc2026_shapefiles", filename: "JRTokyoSta_odc2026_shapefiles.zip", entries: ODC_ENTRIES, unavailable: null, rows_skipped: [] },
    qgis_project: { format: "qgis_project", filename: "JRTokyoSta_qgis_project.zip", entries: ["JRTokyoSta_qgis.qgz", ...ODC_ENTRIES], unavailable: null, rows_skipped: [] }
  };
...
function input(overrides: Partial<DeliverInput> = {}): DeliverInput {
  return {
    ...
    checking: false,
    geoPackage: false,
    options: defaultShapefileOptions(null, ["JRTokyoSta_B1_Space", "JRTokyoSta_1_Space"]),
    ...overrides
  };
}
...
describe("treeLines", () => {
  test("puts the IMDF feature files on one line", () => {
    expect(treeLines(["manifest.json", "venue.geojson", "unit.geojson"])).toEqual(["manifest · venue · unit"]);
  });

  test("names ODC's floors and the layers they carry, from the real file names", () => {
    expect(treeLines(ODC_ENTRIES)).toEqual([
      "_Site · _Building",
  ...
  test("lists the QGIS project file above the ODC files it comes with", () => {
    expect(treeLines(["JRTokyoSta_qgis.qgz", ...ODC_ENTRIES])[0]).toBe("JRTokyoSta_qgis.qgz");
  });
});
...
describe("buildDeliverView", () => {
  ...
  test("the tree, coordinate systems and button follow what is chosen", () => {
    const view = buildDeliverView(input({ selected: new Set<ExportFormat>(["qgis_project", "imdf", "odc2026_shapefiles"]) }));
  ...
  test("the ODC files need a prefix before anything is created", () => {
    const options = { ...input().options, prefix: " " };
    const view = buildDeliverView(input({ selected: new Set<ExportFormat>(["odc2026_shapefiles"]), options }));
  ...
  test("a stale validation says so and offers Check again", () => {
    ...
    expect(view.lastChecks).toEqual([{ tone: "stale", text: expect.objectContaining({ en: "Not checked since the last change" }) }]);
  });

  test("last checks are the stored blockers and can-wait counts, plus rows ODC leaves out", () => {
    const skipped = contents({ odc2026_shapefiles: { rows_skipped: [{ layer: "JRTokyoSta_1_Space" }, { layer: "JRTokyoSta_1_Space" }] } });
    const view = buildDeliverView(
      input({ checks: { error_count: 2, warning_count: 1 }, contents: skipped, selected: new Set<ExportFormat>(["qgis_project"]) })
    );
...
describe("shapefileRequest", () => {
  const options = defaultShapefileOptions(
    { mappings: { unit: { code_column: "CODE" } }, company_mappings: { B0001: "room", B0002: "Room" } } as never,
    ["JRTokyoSta_B1_Space", "JRTokyoSta_1_Opening"]
  );
  ...
  test("round-trip exports overwrite the mapped code column by default", () => {
    ...
    expect(shapefileRequest("shapefiles", options)).toEqual({
      request: {
        ...
        include_report: true,
        unit: { write_imdf_category: true, imdf_category_field: "CODE", overwrite_legacy_code_field: null, legacy_code_map: {} }
      }
    });
  });
  ...
  test("a legacy field must differ from the category field and its mappings must parse", () => {
    ...
    expect(shapefileRequest("shapefiles", { ...options, legacyCodeField: "OLD", legacyMapText: "nonsense" })).toHaveProperty("error");
  });

  test("ODC and QGIS send the prefix; IMDF sends nothing", () => {
    expect(shapefileRequest("qgis_project", options)).toMatchObject({ request: { profile: "odc2026", export_name: "JRTokyoSta" } });
